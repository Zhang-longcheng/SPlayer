...
import { type ExtractPublicPropTypes, type PropType, type Ref, type ShallowRef } from "vue";

/**
 * Props 配置对象
 */
export const lyricPlayerProps = {
  /**
...
// 使用 defineProps 接收上面定义的 props 对象
const props = defineProps(lyricPlayerProps);

// 使用 defineEmits 接收上面定义的 emits 对象
const emit = defineEmits(lyricPlayerEmits);

// 模板引用，对应 template 中的 ref="wrapper-ref"
...
<template>
  <div ref="wrapper-ref" v-bind="$attrs">
    <!-- 如果底部行元素存在，则将 slot 内容传送过去 -->
    <Teleport v-if="bottomLineEl" :to="bottomLineEl">
      <slot name="bottom-line"></slot>
    </Teleport>
  </div>
</template>
